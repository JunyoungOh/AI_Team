...
- **이 앱은 사용자의 컴퓨터에서 로컬로 실행됩니다.** 웹 배포, 서버 호스팅, 모바일 앱이 아닙니다.
- 브라우저에서 HTML 파일을 열거나, 터미널에서 python3 명령어로 실행하는 방식입니다.

## 질문 목적
사용자의 아이디어를 듣고, 로컬 앱 개발에 필요한 핵심 정보를 알아내기 위한 질문을 3~5개 생성하세요.

## 질문 카테고리 (이 중에서 적절한 것만 선택)
1. **핵심 기능**: 이 앱에서 가장 중요한 기능이 뭔지
2. **사용 방식**: 혼자 쓸 건지, 데이터를 저장해야 하는지
3. **데이터**: 어떤 정보를 다루는지
4. **참고 이미지**: 원하는 느낌이나 비슷한 앱이 있는지
5. **특별 요구사항**: 꼭 있어야 할 것 / 없어야 할 것

## 질문하면 안 되는 것 (로컬 앱이므로 불필요)
- 어떤 기기에서 쓸 건지 (항상 사용자의 컴퓨터)
- 웹/모바일/데스크톱 중 어떤 것인지 (항상 로컬)
- 배포 방법이나 도메인
- 로그인/회원가입 필요 여부 (로컬 앱이므로 불필요)
