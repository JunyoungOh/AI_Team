...
# ── Overtime CRUD (야근팀 — 목표 도달까지 반복 실행) ──


def save_overtime(user_id: str, overtime: dict[str, Any]) -> dict[str, Any]:
    """야근팀 작업 저장."""
    if not overtime.get("id"):
        overtime["id"] = _gen_id("overtime")
    if not overtime.get("created_at"):
        overtime["created_at"] = _now_iso()
    overtime["updated_at"] = _now_iso()

    d = _user_dir(user_id) / "overtimes"
    d.mkdir(exist_ok=True)
    path = _safe_path(d, f"{overtime['id']}.json")
    path.write_text(json.dumps(overtime, ensure_ascii=False, indent=2), encoding="utf-8")
    return overtime


def load_overtime(user_id: str, overtime_id: str) -> dict[str, Any] | None:
    d = _user_dir(user_id) / "overtimes"
    path = _safe_path(d, f"{overtime_id}.json")
    if not path.exists():
        return None
    return json.loads(path.read_text(encoding="utf-8"))


def list_overtimes(user_id: str) -> list[dict[str, Any]]:
    d = _user_dir(user_id) / "overtimes"
    if not d.exists():
        return []
    overtimes = []
    for f in sorted(d.glob("overtime_*.json"), reverse=True):
        try:
            overtimes.append(json.loads(f.read_text(encoding="utf-8")))
        except (json.JSONDecodeError, OSError):
            continue
    return overtimes


def update_overtime_iteration(
    user_id: str, overtime_id: str,
    iteration: dict[str, Any],
    status: str | None = None,
) -> dict[str, Any] | None:
    """iteration 결과를 추가하고 상태를 업데이트."""
    ot = load_overtime(user_id, overtime_id)
    if not ot:
        return None
    if "iterations" not in ot:
        ot["iterations"] = []
    ot["iterations"].append(iteration)
    ot["current_iteration"] = len(ot["iterations"])
    if status:
        ot["status"] = status
    ot["updated_at"] = _now_iso()
    return save_overtime(user_id, ot)


def delete_overtime(user_id: str, overtime_id: str) -> bool:
    d = _user_dir(user_id) / "overtimes"
    path = _safe_path(d, f"{overtime_id}.json")
    if path.exists():
        path.unlink()
