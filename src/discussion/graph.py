...
Flow:
  ...
  → [moderator_turn ↔ agent_speak / search_speak / human_turn] loop → report
"""
...
from src.discussion.nodes.search_speak import search_speak
...
def route_after_moderator(state: DiscussionState) -> str:
    """Route: 4-way branch after moderator decision."""
...
def _build_discussion_engine() -> PipelineEngine:
    ...
    engine.add_node("search_speak", search_speak)
    ...
    engine.set_router("search_speak", _route_after_speak)
